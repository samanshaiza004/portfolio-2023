...
const $$projects = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.params;
  const { name, desc, img } = Astro2.props;
  return renderTemplate`${renderComponent($$result, "MainLayout", $$MainLayout, {}, { "default": ($$result2) => renderTemplate`${maybeRenderHead()}<section><img class="rounded"${addAttribute(img, "src")} width="400" height="400"><h1 class="font-bold text-6xl mt-6">${name}</h1><p class="mt-6">${desc}</p></section>` })}`;
